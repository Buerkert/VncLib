	...
	public class VncClient
	{
		...
		public bool Connect(string host, int display, int port, bool viewOnly)
		{
			...
			try {
				...
				if (_rfb.ServerVersion == 0.0) {
					_rfb.WriteProxyAddress();
				...
				var types = _rfb.ReadSecurityTypes();
			    ...
			    Debug.Assert(_securityType > 0, "Unknown Security Type(s)", "The server sent one or more unknown Security Types.");
						
			    _rfb.WriteSecurityType(_securityType);
						
			    // Protocol 3.8 states that a SecurityResult is still sent when using NONE (see 6.2.1)
			    if (_rfb.ServerVersion != 3.8f || _securityType != 1) return _securityType > 1;
...
//					|| types[i] == 18 	// TLS
		...
		{
			var challenge = _rfb.ReadSecurityChallenge();
		...
		public void Initialize()
		{
			...
			Framebuffer = _rfb.ReadServerInit();
			_rfb.WriteSetPixelFormat(Framebuffer);	// just use the server's framebuffer format

			_rfb.WriteSetEncodings(new uint[] {	Rfb.ZRLE_ENCODING,
			                                    Rfb.HEXTILE_ENCODING, 
											//	RfbProtocol.CORRE_ENCODING, // CoRRE is buggy in some hosts, so don't bother using
												Rfb.RRE_ENCODING,
												Rfb.COPYRECT_ENCODING,
												Rfb.RAW_ENCODING });
			
			// Create an EncodedRectangleFactory so that EncodedRectangles can be built according to set pixel layout
			_factory = new EncodedRectangleFactory(_rfb, Framebuffer);
		}
                    ...
                    switch (_rfb.ReadServerMessageType()) {
                        case Rfb.FRAMEBUFFER_UPDATE:
                            ...
                            for (var i = 0; i < rectangles; ++i) {
                                ...
                                _rfb.ReadFramebufferUpdateRectHeader(out Rectangle rectangle, out int enc);

                                // Build a derived EncodedRectangle type and pull-down all the pixel info
                                var er = _factory.Build(rectangle, enc);
                                er.Decode();
                        ...
                        case Rfb.SERVER_CUT_TEXT:
                            ...
                            Clipboard.SetDataObject(_rfb.ReadServerCutText().Replace("\n", Environment.NewLine), true);
                            OnServerCutText();
                            break;
...
// There is no managed way to get a system beep (until Framework v.2.0). So depending on the platform, something external has to be called.
		...
		public void WriteKeyboardEvent(uint keysym, bool pressed)
		{
			try {
                if(!IsViewOnly)
				    _rfb.WriteKeyEvent(keysym, pressed);
			} catch {
				OnConnectionLost();
			}
		}
		...
		{
			try {
				_rfb.WritePointerEvent(buttonMask, point);
			} catch {
    			OnConnectionLost();
			}
		}
		...
		{
			try {
				_rfb.WriteFramebufferUpdateRequest(0, 0, (ushort) Framebuffer.Width, (ushort) Framebuffer.Height, !refreshFullScreen);
			} catch {
				OnConnectionLost();
			}
		}
